...
const currentProduct = selectedProduct || productData;
...
const oldPrice = Number(currentProduct?.oldPrice);
  ...
  return (
    ...
    <div className="min-h-screen bg-gray-50 py-8 px-4">
      <div className="max-w-8xl mx-auto bg-white p-6  grid md:grid-cols-2">
        ...
        <div className="flex gap-8">
          ...
          <div className="relative w-[550px] aspect-[3/4] h-[600px] border rounded-xl overflow-hidden">
            <Image src={selectedImage} alt="product" fill className="object-cover object-[500%_20%]"
            />
          </div>
        </div>
        ...
        {/* RIGHT DETAILS */}
        <div className="space-y-5">
          <h1 className="font-inter font-medium w-[569px] text-[24px] leading-[38px] tracking-normal capitalize text-[#292929]">{currentProduct.title || currentProduct.name}</h1>

          <div className="flex items-center gap-3">
            <span className="text-4xl font-bold">₹ {(currentProduct.price).toLocaleString()}</span>
            <span className="font-inter font-normal text-[24px] leading-[32px] tracking-normal align-middle line-through text-gray-400">₹ {(currentProduct.oldPrice).toLocaleString()}</span>
            <span className="text-[#00560A] font-inter font-semibold text-[20px] leading-[32px] tracking-normal align-middle">({product.discount}% off)</span>
          </div>
           
          <div className="text-xs mb-3 capitalize">
            <p>{currentProduct.taxe || productData.taxe}</p>
          </div>
    ...
    <span className="font-normal text-[15px] leading-[22px] tracking-[-0.01em] capitalize underline">
       {currentProduct.rating} ({currentProduct.reviews || 0} Reviews)
        ...
        <p className="text-red-600 text-md font-bold">Only {currentProduct.stockLeft || productData.stockLeft} left!</p>
